...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data } = await client.from("cases").select("id,merchant_name,problem,status,amount,currency,created_at").order("created_at", { ascending: false });
  ...
  return <main className="min-h-screen bg-[#f4f8f6] text-[#13231f]">
    ...
    <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14">
      ...
      <section className="mt-8 rounded-2xl border bg-white p-5 sm:p-7"><div className="flex items-center justify-between"><div><h2 className="text-xl font-semibold">Your cases</h2><p className="mt-1 text-sm text-[#6c7c77]">Only you can view these records.</p></div></div>
        {cases.length === 0 ? <div className="grid place-items-center py-16 text-center"><span className="grid size-14 place-items-center rounded-2xl bg-[#e9f5f1] text-[#0b755a]"><FolderOpen className="size-6" /></span><h3 className="mt-5 text-lg font-semibold">No recovery cases yet</h3><p className="mt-2 max-w-sm text-sm leading-6 text-[#6b7b76]">Start with a free charge assessment. You’ll see the likely route before deciding whether to continue.</p><Button asChild variant="outline" className="mt-5 rounded-xl"><a href="/#top">Check my first charge <ArrowRight className="ml-2 size-4" /></a></Button></div> : <div className="mt-5 divide-y">{cases.map(item => <article key={item.id} className="flex items-center justify-between gap-4 py-4"><div><h3 className="font-semibold">{item.merchant_name || "Unidentified charge"}</h3><p className="mt-1 text-sm capitalize text-[#6b7b76]">{item.problem.replaceAll("_", " ")} · {item.status.replaceAll("_", " ")}</p></div><p className="font-semibold">{item.currency} {item.amount?.toFixed(2)}</p></article>)}</div>}
      </section>
    </div>
  </main>;
}
