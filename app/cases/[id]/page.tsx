...
export default function CasePage() {
  ...
  const loadCase = useCallback(async () => {
    ...
    const [{ data: caseData, error: caseError }, { data: documentData, error: documentError }, { data: fieldData }] = await Promise.all([
      supabase.from("cases").select("id,merchant_name,problem,status,recovery_type,amount,currency,charge_date,strength_score,strength_label").eq("id", caseId).single(),
