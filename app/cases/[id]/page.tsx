...
import { ArrowLeft, CheckCircle2, Download, FileText, LockKeyhole, Route, UploadCloud } from "lucide-react";
...
export default function CasePage({ params }: { params: { id: string } }) {
  ...
  const loadCase = useCallback(async () => {
    ...
    const [{ data: caseData, error: caseError }, { data: documentData, error: documentError }] = await Promise.all([
      ...
      supabase.from("documents").select("id,original_filename,content_type,size_bytes,object_path,created_at").eq("case_id", params.id).order("created_at", { ascending: false }),
    ]);
