...
export default function AuthPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (mode === "signup") {
        ...
        if (data.session) window.location.replace("/dashboard");
        else setMessage("Check your email to confirm your RefundRoute account.");
      } else {
  ...
  return <main className="grid min-h-screen bg-[#f4f8f6] lg:grid-cols-[.9fr_1.1fr]">
    <section className="hidden bg-[#102d25] p-12 text-white lg:flex lg:flex-col lg:justify-between">
      <a href="/" className="flex items-center gap-2.5 text-lg font-semibold"><span className="grid size-10 place-items-center rounded-xl bg-[#8de0c6] text-[#10362b]"><Route className="size-5" /></span>RefundRoute</a>
    ...
    <section className="flex items-center justify-center px-5 py-10 sm:px-8">
      <div className="w-full max-w-md">
        <a href="/" className="mb-10 inline-flex items-center gap-2 text-sm font-medium text-[#587069] hover:text-[#0b6b53]"><ArrowLeft className="size-4" />Back to RefundRoute</a>
        ...
        <p className="mt-8 text-center text-xs leading-5 text-[#7b8b85]">By continuing, you agree to RefundRoute’s Terms and Privacy Policy.</p>
      </div>
    </section>
  </main>;
}
