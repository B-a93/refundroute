...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f7f9f8] text-[#13231f]">
      ...
      <section id="top" className="relative">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-5 py-14 sm:px-8 sm:py-20 lg:grid-cols-[.9fr_1.1fr] lg:items-center lg:py-24">
          <div className="max-w-xl">
            ...
            <h1 className="text-[clamp(2.5rem,6vw,4.9rem)] font-semibold leading-[.98] tracking-[-0.055em] text-[#10251f]">Charged for something you didn’t expect?</h1>
            <p className="mt-7 max-w-lg text-lg leading-8 text-[#52655f]">Let’s work out what happened, who processed the payment and what evidence can strengthen your refund request.</p>
          ...
          <div id="assessment" className="relative scroll-mt-24 rounded-[28px] border border-[#d7e2de] bg-white p-5 shadow-[0_30px_80px_rgba(24,57,48,.12)] sm:p-8">
            ...
            {!showResult ? (
              <div className="space-y-5">
                ...
                <p className="text-center text-xs leading-5 text-[#74837e]">We help you understand your options and prepare your case. We do not process refunds or guarantee approval.</p>
              </div>
            ) : (
      ...
      <section className="py-16 sm:py-20"><div className="mx-auto grid max-w-6xl gap-9 px-5 sm:px-8 lg:grid-cols-[.8fr_1.2fr] lg:items-center">
        <div><p className="text-sm font-bold uppercase tracking-[.13em] text-[#0b8062]">A clearer first answer</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">See what your assessment can reveal.</h2><p className="mt-5 leading-7 text-[#5b6d67]">Your result turns the details you provide into a practical evidence check and a recommended route. Every case is different; this fictional example shows the format.</p></div>
        <article className="rounded-[28px] border border-[#c8ddd5] bg-white p-5 shadow-[0_24px_60px_rgba(24,57,48,.1)] sm:p-7">
          <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.13em] text-[#0b8062]">Example assessment</p><h3 className="mt-1.5 text-xl font-semibold">Unexpected Apple subscription renewal</h3></div><span className="rounded-full bg-[#e8f6f1] px-3 py-1.5 text-sm font-semibold text-[#17624f]">Evidence: 82/100</span></div>
          <dl className="mt-6 grid gap-3 sm:grid-cols-2">
            {[['Likely payment route','Apple'],['Strongest evidence','Cancellation confirmation and receipt'],['Evidence still needed','Screenshot showing cancellation date'],['Recommended next step','Request the refund through Apple']].map(([label,value]) => <div key={label} className="rounded-xl bg-[#f5f8f7] p-4"><dt className="text-xs font-semibold uppercase tracking-wide text-[#71817c]">{label}</dt><dd className="mt-1.5 font-semibold text-[#203a32]">{value}</dd></div>)}
          </dl>
          <div className="mt-4 flex gap-3 rounded-xl border border-[#d8e5e0] p-4"><Sparkles className="mt-0.5 size-5 shrink-0 text-[#0b8062]" /><div><p className="font-semibold">With the $9 guided case</p><p className="mt-1 text-sm leading-6 text-[#60716b]">Generate an editable request based on your confirmed facts, then follow the company-specific submission steps.</p></div></div>
        </article>
      </div></section>
        ...
        <div className="mx-auto flex max-w-5xl flex-col items-center px-5 text-center sm:px-8">
          ...
          <div className="mt-9 grid w-full gap-4 text-left md:grid-cols-2 md:items-start">
            ...
            <div className="rounded-2xl border border-[#71d3b6]/50 bg-[#174538] p-6 shadow-xl">
              ...
              <p className="mt-5 font-semibold text-[#e5f4ef]">Your $9 guided case includes:</p>
              <ul className="mt-3 space-y-3 text-sm leading-6 text-[#c5ddd6]">
                {["A personalized refund request based on confirmed facts", "An editable draft you can save and copy", "Company-specific submission instructions", "A direct link to the official support route", "Your evidence checklist and strength assessment", "Your private case record and evidence in one place", "Recommended follow-up actions for the case"].map(item => <li key={item} className="flex gap-2"><Check className="mt-1 size-4 shrink-0 text-[#8de0c6]" />{item}</li>)}
              </ul>
              ...
              <p className="mt-3 text-xs leading-5 text-[#a9c9bf]">You pay only after seeing your free assessment. No percentage is taken from your refund. Payment does not guarantee that the company will approve your request.</p>
            </div>
          </div>
